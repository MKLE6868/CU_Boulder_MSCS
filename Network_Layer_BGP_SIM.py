...
class Router:
    ...
    def update(self, rt):
        ...
        if key not in self.rib:
            ...
            updated = False
            for i, existing in enumerate(self.rib[key]):
                if existing.neighbor == rt.neighbor:
                    self.rib[key][i] = rt
                    updated = True
                    break
                if not updated:
    ...
    def withdraw(self, rt):
        ...
        key = rt.pfx_str()
        if key in self.rib:
            self.rib[key] = [r for r in self.rib[key] if r.neighbor != rt.neighbor]
            if len(self.rib[key]) == 0:
                del self.rib[key]
    ...
    def next_hop(self, ipaddr):
        ...
        for key, routes in self.rib.items():
            pfx, plen = key.split('/')
            plen = int(plen)
            pfx_bin = self.convertToBinaryString(pfx)

            if ip_bin.startswith(pfx_bin[:plen]):
                if plen > best_match_len:
                    best_match_len = plen
                    best_routes = routes
                elif plen == best_match_len:
                    best_route += routes

        if best_routes:
            best_route = min(best_routes, key=lambda r: len(r.path))
